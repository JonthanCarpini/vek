...
export const deliveryApi = {
  info: (slug: string) => call<any>(`/api/v1/delivery/${slug}/info`),
  menu: (slug: string) => call<{ categories: any[] }>(`/api/v1/delivery/${slug}/menu`),
  requestOtp: (slug: string, phone: string) =>
    call<{ expiresAt: string; debugCode?: string }>(`/api/v1/delivery/auth/request-otp`, {
      method: 'POST',
      body: JSON.stringify({ slug, phone }),
    }),
  verifyOtp: (slug: string, phone: string, code: string, name?: string) =>
    call<{ token: string; customer: any }>(`/api/v1/delivery/auth/verify-otp`, {
      method: 'POST',
      body: JSON.stringify({ slug, phone, code, name }),
    }),
