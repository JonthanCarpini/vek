...
export default function LoginStep() {
  const { slug, loginSuccess, goTo } = useDelivery();
  ...
  const handleRequestOtp = async () => {
    ...
    const res = await deliveryApi.requestOtp(slug, cleanPhone);
    setLoading(false);
    if (!res.ok) {
      setError(res.error);
      return;
  ...
  const handleVerify = async () => {
    ...
    const cleanPhone = phone.replace(/\D/g, '');
    const res = await deliveryApi.verifyOtp(slug, cleanPhone, code, name.trim());
    setLoading(false);
    if (!res.ok) {
      setError(res.error);
      return;
