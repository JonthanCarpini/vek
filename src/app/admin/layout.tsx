    ...
    <div className="min-h-screen flex flex-col">
                ...
                {group.items.map((n) => {
                  ...
                  return (
                    <Link
                      ...
                      href={n.href}
                      onClick={() => setIsSidebarOpen(false)}
                      className={`px-3 py-3 rounded-xl text-sm transition-all duration-200 flex items-center gap-3 touch-manipulation min-h-[48px] ${active ? 'bg-brand-600 text-white shadow-lg shadow-brand-600/30' : 'hover:bg-white/5 active:bg-white/10 text-gray-400 hover:text-white'}`}
                    >
      ...
      >
        ...
        <button
          ...
          className={`flex-1 flex flex-col items-center justify-center py-2 gap-0.5 touch-manipulation min-h-[56px] transition-colors ${isSidebarOpen ? 'text-brand-400' : 'text-gray-500 active:text-gray-300'}`}
        >
